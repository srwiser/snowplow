  ...
  "The collector service" should {
    "cookie" in {
      "attach p3p headers" in {
        val (r, l) = service
          .cookie("nuid=12", Some("b"), "p", None, None, None, "h", RemoteAddress.Unknown, HttpRequest(), false)
      ...
      "not store stuff if bouncing and provide a location header" in {
        val (r, l) = bouncingService
          .cookie("", Some("b"), "p", None, None, None, "h", RemoteAddress.Unknown, HttpRequest(), true)
      ...
      "store stuff if having already bounced with the fallback nuid" in {
        val (r, l) = bouncingService
          .cookie("bounce=true", Some("b"), "p", None, None, None, "h", RemoteAddress.Unknown, HttpRequest(), true)
        r.headers must have size 4
        l must have size 1
    ...
    "buildEvent" in {
      "fill the correct values" in {
        val l = `Location`("l")
        val ct = `Content-Type`(ContentType(MediaTypes.`image/gif`))
        val r = HttpRequest().withHeaders(l :: ct :: hs)
        val e = service
          .buildEvent("q", Some("b"), "p", Some("ua"), Some("ref"), "h", "ip", r, "nuid")
        ...
        e.contentType shouldEqual ct.value.toString
      }
    }
    ...
    "contentType" in {
      "return the content-type header if present" in {
        val request = HttpRequest()
          .withHeaders(`Content-Type`(ContentType(MediaTypes.`application/json`)))
        service.contentType(request) shouldEqual Some("application/json")
      }
      "return None if the content-type header is absent" in {
        service.contentType(HttpRequest()) shouldEqual None
      }
    }
