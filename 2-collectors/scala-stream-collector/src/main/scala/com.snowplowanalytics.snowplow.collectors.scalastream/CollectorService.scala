...
class CollectorService(
  ...
  override def cookie(
    ...
    val (ipAddress, partitionKey) = ipAndPartitionKey(ip, config.sink.useIpAddressAsPartitionKey)
  ...
  def buildEvent(
    ...
    e.headers = headers(request).asJava
    contentType(request).foreach(e.contentType = _)
    e
  }
   ...
   */
  def contentType(request: HttpRequest): Option[String] =
