...
class CollectorService(
  ...
  private val collector =
    s"${Settings.shortName}-${Settings.version}-${config.sink.enabled.toString.toLowerCase}"
  ...
  override def cookie(
    ...
    val (ipAddress, partitionKey) = getIpAndPartitionKey(ip, config.sink.useIpAddressAsPartitionKey)
    val nuid = getNetworkUserId(request, cookie)
    val event =
      buildEvent(queryString, body, path, userAgent, refererUri, hostname, ipAddress, request, nuid)
  ...
  def buildEvent(
    ...
    e.headers = getHeaders(request).asJava
    getContentType(request).foreach(e.contentType = _)
    e
  }
  ...
  /**
   * Extracts the content type header from a request.
   * @param request containing headers
   * @return an optional lower-cased content type header
   */
  def getContentType(request: HttpRequest): Option[String] =
